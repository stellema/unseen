...
penalty = np.log(np.finfo(float).max) * 100
...
def fit_gev(
    ...
    stationary : bool, default True
        Fit as a stationary GEV using scipy.stats.genextreme.fit
    covariate : array_like, default None
    ...
    assert_good_fit : bool, default False
        Return NaNs if any of the parameters fall outside the support of the
        distribution (stationary only)
        goodness of fit test (see `goodness_of_fit_kwargs`) at `alpha` level.
    ...
    minimizer_kwargs : dict, default {'method': 'Nelder-Mead'}
        Additional keyword arguments to pass to `scipy.optimize.minimize`
        (excluding `x0`, `args`, `data`, `bounds`)
    use_basinhopping : bool, default False
        Use `scipy.optimize.basinhopping` to run the minimizer using random
        perturbations of the initial guess to help find the global minimum.
        This is slow, but useful if results are sensitive to the initial guess
    basinhopping_kwargs : dict, optional
        Additional keyword arguments to pass to `scipy.optimize.basinhopping`.
        Important options include `stepsize`, `niter`, `rng` and `T`. The
        algorithm generates `interval` initial guesses with random displacements
        between +/- `stepsize` (larger stepsizes promote exploration; e.g.,
        shape param +/- `stepsize`). If the acceptance rate of the interval is
        below (above) `target_accept_rate`, the stepsize is increased (decreased)
        by `stepwise_factor`. The global minimum is based on minimum of `niter`
        runs of the local minimizer as defined in `minimizer_kwargs`.
    ...
    - For stationary=True, stationary parameters are estimated using
    `scipy.stats.genextreme.fit` with initial guess based on `fitstart` (use
     fitstart 'scipy_fitstart' or None to use scipy defaults).
    - For stationary=True, the parameters (including the linear location and
    scale trend parameters) are estimated by minimising a penalised negative
    log-likelihood function.
    - The location and/or scale trend parameters can be fixed by setting
    `loc1=None` and `scale1=None`.
    - To optimize stationary parameters using the custom minimizer and
      basinhopping options, use `stationary=False` with fixed trend parameters.
    - The Gumbel distribution can be estimated by setting `fc=0`.
    ...
    - The `assert_good_fit` will return NaNs if the parameters don't pass a
    goodness of fit test (see `check_gev_fit`).
    ...
    def _fit_1d(
        ...
        if isinstance(fitstart, str):
            ...
            if np.isnan(dparams_i).any():
                # If the fitstart method fails, return NaNs
                return np.full(n, np.nan)
        else:
        ...
        if stationary or pick_best_model:
            if fitstart is None:
                dparams = genextreme.fit(data, **scipy_fit_kwargs)
            else:
        ...
        if not stationary:
            ...
            for i, trend_param in zip([2, 4], [loc1, scale1]):
                if trend_param is None:
                    dparams_ns_i[i] = 0
                    bounds[i] = (0, 0)
                ...
                res = basinhopping(
                    ...
                    **basinhopping_kwargs,
                    minimizer_kwargs=dict(
                        args=(data, covariate), bounds=bounds, **minimizer_kwargs
                    ),
            ...
            if not isinstance(pick_best_model, str):
                dparams = dparams_ns
            else:
                # Degrees of freedom for relative goodness of fit test
                # (stationary and nonstationary models)
                n_params = [3, 5]
                for fixed in [fc, floc, fscale]:
                    if fixed is not None:
                        # Reduce the degrees of freedom if a parameter is fixed
                        n_params[0] -= 1
                        n_params[1] -= 1
                # Reduce degrees of freedom for fixed trend parameters
                if loc1 is None:
                    n_params[1] -= 1
                if scale1 is None:
                    n_params[1] -= 1
        ...
        if assert_good_fit:
            nll = _gev_nllf(dparams, data, covariate)
            # Test stationary goodness of fit
            pvalue = np.nan  # Ignore p-value if not calculated
            if stationary:
                pvalue = check_gev_fit(data, dparams, **goodness_of_fit_kwargs)
            elif len(dparams) == 5 and dparams[2] == 0 and dparams[4] == 0:
                _dparams = dparams[np.array([0, 1, 3])]  # Remove trend elements
                pvalue = check_gev_fit(data, _dparams, **goodness_of_fit_kwargs)
...
def _gev_nllf(dparams, x, covariate=0):
    ...
    Notes
    -----
    THe shape parameter does not follow consistency with scipy.stats results.
    ...
    if shape == 0.0:
        L = np.where(valid, np.log(scale) + s + np.exp(-s), np.inf)
...
def _fitstart_1d(data, method, scipy_fit_kwargs={}):
    ...
    scipy_fit_kwargs : dict, optional
        Additional keyword arguments to pass to `scipy.stats.genextreme.fit`
    ...
    if method == "LMM":
        ...
        try:
            ...
            warnings.warn(
                "L-moments failed to generate a initial guess for MLE. "
                + f"Considering changing the fitstart method.{e}"
            )
            return e
        ...
        # MLE
        dparams_i = genextreme.fit(data, **scipy_fit_kwargs)

    elif method == "scipy_subset":
        # MLE (equivalent of fitstart='scipy_subet')
        dparams_i = genextreme.fit(data[::2], **scipy_fit_kwargs)

    elif method == "xclim_fitstart":
        # Approximates the probability weighted moments (PWM) method?
        args, kwargs = xcstats._fit_start(data, dist="genextreme", **scipy_fit_kwargs)
...
class RandomDisplacementBounds(object):
    """Basinhopping random step with bounds.

    Modified from: https://stackoverflow.com/a/21967888/2320035
    """

    def __init__(self, xmin, xmax, stepsize=0.5, rng=None):
        self.xmin = xmin
        self.xmax = xmax
...
def check_gev_relative_fit(data, nll_1, nll_2, test, alpha=0.05, n_params=[3, 5]):
    """Test if the second model is a better fit than the first model.
    ...
    data : array_like
        Data to use in estimating the distribution parameters
    nll_1, nll_2 : float
        Negative log-likelihood of the models
    test : {"AIC", "AICc", "BIC", "LRT"}
        Method to test relative fit of the models (case insensitive)
    alpha : float, default 0.05
        Significance level for the likelihood ratio test
    n_params : list, default [3, 5]
    ...
    Likelihood ratio test (LRT):
        - The null hypothesis is that the simpler model is correct.
    ...
    test = test.upper()  # convert str to uppercase
    # Degrees of freedom of each model
    if test == "LRT":
        # Likelihood ratio test statistic
        LR = -2 * (nll_2 - nll_1)
        result = chi2.sf(LR, n_params[1] - n_params[0]) <= alpha

    elif test in ["AIC", "AICC"]:
        # Akaike Information Criterion (AIC)
        aic = [(2 * k) + (2 * nll) for nll, k in zip([nll_1, nll_2], n_params)]
        if test == "AICC":
            # Akaike Information Criterion  with correction (AICc)
            diff = [(2 * k * (k + 1)) / (len(data) - k - 1) for k in n_params]
            aic = [a + d for a, d in zip(aic, diff)]
        result = aic[0] > aic[1]

    elif test == "BIC":
        # Bayesian Information Criterion (BIC)
        bic = [
            k * np.log(len(data)) + (2 * nll)
            for nll, k in zip([nll_1, nll_2], n_params)
        ]
        result = bic[0] > bic[1]
    else:
...
def get_best_GEV_model_1d(
    data, dparams, dparams_ns, covariate, alpha, test, n_params=[3, 5]
):
    ...
    nll_1 = _gev_nllf([shape, loc, scale], data)
    nll_2 = _gev_nllf(dparams_ns, data, covariate)

    result = check_gev_relative_fit(
        data, nll_1, nll_2, test=test, alpha=alpha, n_params=n_params
    )
    if not result:
...
def spatial_plot_gev_parameters(
    ...
    if dataset_name:
        fig.suptitle(f"{dataset_name} GEV parameters", y=0.77 if stationary else 0.96)
